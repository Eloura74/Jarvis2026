...
const DEFAULT_APP_PATHS: AppPath[] = [
  ...
  },
  {
    ...
    aliases: ['cmd', 'command', 'terminal', 'invite de commandes', 'command prompt'],
    category: 'system',
    icon: '⌨️'
  },
  {
  ...
  // ============================================================================
  {
    ...
    aliases: ['bambu', 'bambu studio', 'bambu slicer'],
    category: 'other',
    icon: '🖨️'
  },
  {
  ...
  },
  {
    ...
    aliases: ['vlc', 'media player', 'lecteur vidéo', 'vlc player', 'video player'],
    category: 'media',
    icon: '🎬'
  },
  {
...
export const useAppPaths = () => {
  ...
  const findApp = useCallback((query: string): AppPath | null => {
    ...
    app = appPaths.find(a => 
      ...
      a.aliases.some(alias => alias.toLowerCase().startsWith(normalizedQuery))
    );
    if (app) return app;
    ...
    app = appPaths.find(a => 
      ...
      a.aliases.some(alias => alias.toLowerCase().includes(normalizedQuery))
    );
  ...
  const updateApp = useCallback((index: number, updatedApp: AppPath) => {
    setAppPaths(prev => prev.map((app, i) => i === index ? updatedApp : app));
  }, []);
