...
const App: React.FC = () => {
  ...
  // Synchroniser wake word avec les settings
  useEffect(() => {
    // Si settings.wakeWordEnabled = true et wake word n'est pas activé, l'activer
    // Si settings.wakeWordEnabled = false et wake word est activé, le désactiver
    if (settings.wakeWordEnabled && !wakeWordEnabled) {
      toggleWakeWord();
    } else if (!settings.wakeWordEnabled && wakeWordEnabled) {
      toggleWakeWord();
    }
  }, [settings.wakeWordEnabled, wakeWordEnabled, toggleWakeWord]);
