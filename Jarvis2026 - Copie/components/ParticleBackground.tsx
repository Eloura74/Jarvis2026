...
const ParticleBackground: React.FC<ParticleBackgroundProps> = ({ status }) => {
  ...
  useEffect(() => {
    ...
    class Particle {
       ...
       * Applique 3 comportements :
       * 1. Vitesse modulée selon le statut (x1 si IDLE, x3 sinon)
       * 2. Rebond sur les bords du canvas
       * 3. Attraction vers le curseur souris si proche (<150px)
       */
      update() {
        // 1. Vitesse selon statut système
        // IDLE = mouvement lent (x1), autres états = mouvement rapide (x3)
        const speedMultiplier = status === SystemStatus.IDLE ? 1 : 3;

        this.x += this.vx * speedMultiplier;
        this.y += this.vy * speedMultiplier;

        // 2. Rebond sur les bords (inversion vélocité)
        if (this.x < 0 || this.x > canvas!.width) this.vx *= -1;
        if (this.y < 0 || this.y > canvas!.height) this.vy *= -1;

        // 3. Interaction souris : attraction douce si distance <150px
