...
import { Menu, Volume2, Mic, Clock, Settings } from "lucide-react";
...
interface TopHUDProps {
  ...
  onOpenHistory: () => void;
  /** Callback appelée quand l'utilisateur ouvre les paramètres */
  onOpenSettings: () => void;
  /** État du listening manuel (reconnaissance vocale active) */
  isListening?: boolean;
  /** Callback appelée quand l'utilisateur toggle le listening manuel */
  ...
  return (
    ...
    <div className="absolute top-0 w-full p-6 flex justify-between items-start z-40">
      ...
      <div className="flex gap-4">
        ...
        {/* Toggle Listening Manuel : Microphone */}
        {onToggleListening && (
          <button
            ...
            className={`p-3 border rounded-full transition-all backdrop-blur-md relative ${
              isListening
                ...
                : "bg-transparent text-cyan-400 border-cyan-500/30 hover:bg-cyan-500/20"
            }`}
            aria-label={isListening ? "Arrêter l'écoute" : "Démarrer l'écoute"}
            title={
              isListening
                ? "Écoute active - Cliquer pour arrêter"
                : "Activer reconnaissance vocale manuelle"
            }
          >
            <Mic className="w-5 h-5" />
            {isListening && (
              <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[8px] text-red-400 whitespace-nowrap uppercase tracking-wider">
                Recording
              </span>
            )}
          </button>
